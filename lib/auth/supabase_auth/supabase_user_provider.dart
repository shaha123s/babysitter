...
class BabysitterSupabaseUser extends BaseAuthUser {
  BabysitterSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> babysitterSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BabysitterSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
